...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus,
                      int clear, int dontuse_opencv, int dont_show, int mjpeg_port,
                      int calc_topk, int show_imgs, char* chart_path)
{
    int i,j;

    float avg_loss = -1;  // 指数滑动平均损失
    ...
    // 第一块显卡上的网络
    network net = nets[0];
    // 实际的batch_size等于batch和subdivisions和gpus数目乘积
    ...
    list *options = read_data_cfg(datacfg);  // 获取训练数据的路径, 存储到链表里面

    char *backup_directory = option_find_str(options, "backup", "/backup/");
    char *label_list = option_find_str(options, "labels", "data/labels.list");  // 类别名列表
    char *train_list = option_find_str(options, "train", "data/train.list");    // train_data
    int classes = option_find_int(options, "classes", 2);  // 类别数
    ...
    char **labels = get_labels(label_list);  // 获取类别名cifar:{"airplane:1","automobile:2","bird:5","cat",...}
    list *plist = get_paths(train_list);     // 这里是将训练文件路径全部载入到list类型的plist中
    char **paths = (char **)list_to_array(plist); // 全部的训练文件路径字符串数组

    // 为了后期能够控制比例, 可以模拟pytorch中加权随机采样.
    // 思路: 需要对labels进行修改, 每个label后面跟一个权重比例, 也即: labels.txt中需要写成如下形式
    // class_name1: 1, class_name2:5..., 如果所有的都没有写, 则默认权重相等.
    // 需要构建变量:
    // 根据labels构建labels_weights:["cls_1", "cls_1", "cls_2","cls_2", "cls_2", "cls_2"]
    // 然后遍历paths, 构建一个链表, 链表的key就是cls_name, value就是所有为这个类的文件路径的字符串数组
    weights label_weights = get_labels_with_weights(labels, classes);  // 权重向量和总数
    sets* sample_dicts = get_samples_set(paths, plist->size, labels, classes);
    labels = remove_weights(labels, classes);

    printf("train_images_num = %d\n", plist->size);
    int train_images_num = plist->size;
    clock_t time;

    load_args args = {0};  // 初始化args
    args.w = net.w;
    args.h = net.h;
    args.c = net.c;
    args.threads = 8;    // 开启线程数
    if (net.contrastive && args.threads > net.batch/2) args.threads = net.batch / 2;   // 这里在控制线程数
    args.hierarchy = net.hierarchy;
    //调节图片旋转角度、曝光度、饱和度、色调等, 来增加图片数量, 数据增强相关配置
    ...
    args.flip = net.flip;
    args.blur = net.blur;
    args.angle = net.angle;    // 图像增强操作, 图像旋转[-angle, +angle]
    args.aspect = net.aspect;  // 宽高比, classifier: aspect = rand_scale(aspect);
    args.exposure = net.exposure;
    args.saturation = net.saturation;
    args.hue = net.hue;
    args.size = net.w > net.h ? net.w : net.h;
    // new_onehot_target_labels = onehot_labels * (1 - label_smooth_eps) + label_smooth_eps / num_classes
    args.label_smooth_eps = net.label_smooth_eps;  // 标签平滑因子
    ...
    args.n = imgs;              // imgs=net.batch*net.subdivisions*ngpus
    args.m = train_images_num;  // train图片总数
    args.labels = labels;       // 标签类别
    args.sample_weight = label_weights;  // 权重随机采样新增
    args.sample_dicts = sample_dicts;    // 权重随机采样新增
    ...
    data train;
    data buffer;
    pthread_t load_thread;  // 线程标识符
    args.d = &buffer;  // args.d中用来装载多线程load的数据
    // 多线程加载数据参看: https://blog.csdn.net/u012927281/article/details/86550702
    load_thread = load_data(args);  // load数据, 同时已经做好data_augumentation
    ...
    start = what_time_is_it_now();  // 开始计时
    // 开始训练
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        time=clock();
        // pthread_join()阻塞, 主函数等待线程load_thread函数执行完毕, 再往下继续执行
        pthread_join(load_thread, 0);  // 等待load_threads()函数执行完毕
        train = buffer;
        load_thread = load_data(args);  // 为下一个iter训练做数据准备, 数据照样装在到buffer中
        // 记录load数据花费的时间
        printf("Data Loaded Consuming: %lf seconds\n", sec(clock()-time));
        time=clock();
...
#else
        loss = train_network(net, train);  // 训练网络开始
#endif
        if(avg_loss == -1 || isnan(avg_loss) || isinf(avg_loss)) avg_loss = loss;
        avg_loss = avg_loss*.9 + loss*.1;  // 这里avg_loss采用指数滑动平均形式, 减少噪声数据
        ...
        fprintf(stderr,"iter=%d, img_percent=%.3f, loss=%f, avg_loss=%f, lr=%f, time_consume=%lf s, seen_imgs=%ld imgs, remainder=%f h\n",
               get_current_batch(net), (float)(*net.seen)/ train_images_num, loss, avg_loss,
               get_current_rate(net), sec(clock()-time), *net.seen, avg_time);
#ifdef OPENCV
        if (net.contrastive) {
            float cur_con_acc = -1;
            int k;
...
void run_classifier(int argc, char **argv)
{
    ...
    char *data = argv[3];  // 获取数据路径
    char *cfg = argv[4];   // 获取网络路径
    char *weights = (argc > 5) ? argv[5] : 0;   // 默认argv[5]放权重路径文件
    char *filename = (argc > 6) ? argv[6]: 0;   // 预测图片文件路径
    ...
    else if(0==strcmp(argv[2], "train")) train_classifier(data, cfg, weights, gpus, ngpus, clear, dontuse_opencv,
