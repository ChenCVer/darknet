...
char **get_random_paths_custom(char **paths, int n, int m, int contrastive)
{
    ...
    for(i = 0; i < n; ++i){
        do {
            int index = random_gen() % m;  // 随机产生一个0~m之间的整数
            if (contrastive && (i % 2 == 1)) {  // 这个contrastive在这里什么意思?
                index = old_index;
            }
            else {
                old_index = index;
...
matrix load_image_augment_paths(char **paths, int n, int use_flip, int min, int max, int w, int h,
                                ...
                                int dontuse_opencv, int contrastive)
{
    ...
    for(i = 0; i < n; ++i){
        int size = w > h ? w : h;  // 取最大边, w和h是最终输入网络的图片尺寸
        image im;  // 一维数组: h*w*c
        const int img_index = (contrastive) ? (i / 2) : i;  // 这是什么意思?
        if(dontuse_opencv) im = load_image_stb_resize(paths[img_index], 0, 0, 3);
        else im = load_image_color(paths[img_index], 0, 0);
        // 这里进行旋转、裁剪(random_augment_image)
        // TODO: 有疑问, 对random_augment_image()函数的aspect参数.
        image crop = random_augment_image(im, angle, aspect, min, max, size);
        //如果开启了多线程,当有多条线程抢占着个资源时,会报错. 解决方法就是,使用单batch=1即可.
        //show_image(im, "orig");
        //show_image(crop, "crop");
        //cvWaitKey(0);
        // TODO: darknet只是加上了水平翻转, 后期考虑加上vflip操作
        int flip = use_flip ? random_gen() % 2 : 0;
        if (flip)  // 水平翻转图像, 后期可以把垂直vlip操作加上.
            flip_image(crop);
        // 这里是进行hue(色调), 曝光(exposure), saturation(饱和度),
        random_distort_image(crop, hue, saturation, exposure);
        // 这里是将图片resize到网络输入的尺寸大小
...
void fill_truth_smooth(char *path, char **labels, int k, float *truth, float label_smooth_eps)
{
    ...
    memset(truth, 0, k * sizeof(float));
    int count = 0;
    for (i = 0; i < k; ++i) {  // 从代码结构中也可以看出, darknet的分类, 数据集的存放形式
        if (strstr(path, labels[i])) {  // strstr是C语言中的函数,作用是返回字符串中首次出现子串的地址.
            truth[i] = (1 - label_smooth_eps);
            ++count;
        }
        else {
            truth[i] = label_smooth_eps / (k - 1);  // 为什么是(k-1)
        }
    }
    if (count != 1) {  // 难道要看绿多标签分类?
        printf("Too many or too few labels: %d, %s\n", count, path);
        count = 0;
        for (i = 0; i < k; ++i) {
            if (strstr(path, labels[i])) {
...
{
    matrix y = make_matrix(n, k);  // n个样本, k为类别的one-hot编码
    int i;
    if (labels) {
        // supervised learning
        for (i = 0; i < n; ++i) {
            ...
            if (hierarchy) {
                fill_hierarchy(y.vals[i], k, hierarchy);  // 这是啥意思?
            }
        }
    } else {
...
char** get_random_weight_sample_paths(weights sample_weight, sets* sample_dicts, int n){
    int i, j, seed, index, sample_id;
    char* classname;
    char** paths = (char**) malloc(n * sizeof(char*));
    for(i = 0; i < n; i++){
        // 随机从sample_weight中选择一个数
        index = random_gen() % sample_weight.weight_sum;
        // 取出对应的类别
        classname = sample_weight.labels_weights[index];
        // 然后在sample_dicts中找对对应的key, 然后随机找到一条数据
        for(j = 0;j < sample_weight.class_num; j++){
            if(0 == strcmp(classname, sample_dicts[j].type)) {
                sample_id = random_gen() % sample_dicts[j].size;
                paths[i] = (char*) malloc(strlen(sample_dicts[j].filepaths[sample_id])*sizeof(char));
                strcpy(paths[i], sample_dicts[j].filepaths[sample_id]);
            }
        }
    }
    // debug
...
// 此函数的功能是实现权重随机采样
weights get_labels_with_weights(char** labels, const int classes) {
    int i = 0, j = 0, num = 0;
    weights sw;
    int* weight = (int*) malloc(classes * sizeof(int));
    char* p = NULL;
    char dst[100] = "";
    // 获取权重总和.
    for(i = 0; i < classes; i++) {
        p = strchr(labels[i], ':');
        strncpy(dst, p+1, strlen(p)-1);
        weight[i] = atoi(dst);
        num += atoi(dst);
    ...
    sw.weight_sum = num;
    // 开辟空间, 构造权重列表.
    char** labels_weights = (char**) malloc(num * sizeof(char*));
    char** ptr = labels_weights;
    for(i = 0; i < classes; i++) {
        for(j = 0; j < weight[i]; j++) {
            p = strchr(labels[i], ':');
            ptr[j] = (char*) malloc(num * sizeof(char));
            memset(ptr[j], 0, num);
            strncpy(ptr[j], labels[i], strlen(labels[i])-strlen(p));
        }
        ptr += weight[i];
    }
    sw.labels_weights = labels_weights;
    sw.class_num = classes;
    if(weight != NULL) free(weight);

    return sw;
}

sets* get_samples_set(char** paths, int train_data_num, char** labels, int class_num){
    int i, j;
    // 用来装在所有的类别以及每个类别对应的全部文件路径
    sets* sample_dicts = (sets*) malloc(class_num * sizeof(sets));
    // 初始化sample_dicts
    char* p = NULL;
    for(i = 0; i < class_num; i++) {
        p = strchr(labels[i], ':');
        sample_dicts[i].type = (char*) malloc(100 * sizeof(char));
        memset(sample_dicts[i].type, 0, 100);
        strncpy(sample_dicts[i].type, labels[i], strlen(labels[i])-strlen(p));
        sample_dicts[i].size = 0;
        sample_dicts[i].filepaths = (char**) malloc(train_data_num*sizeof(char*));
    }

    for (i = 0; i < train_data_num; i++){
        char* _ptr = strchr(paths[i], '_');
        char classname[50];
        strncpy(classname, _ptr+1, strlen(_ptr)-5);
        // 在sample_dicts中查找是否含有classname
        for(j = 0; j < class_num; j++){
            // 找到对应的位置, 将其放入
            if(0 == strcmp(classname, sample_dicts[j].type)){
                sample_dicts[j].filepaths[sample_dicts[j].size] = (char*) malloc(255 * sizeof(char));
                strcpy(sample_dicts[j].filepaths[sample_dicts[j].size], paths[i]);
                sample_dicts[j].size++;
                break;
            }
        }
...
char** remove_weights(char** labels, int classes){
    int i;
    char** new_labels = (char**) malloc(classes * sizeof(char*));
    for(i = 0; i < classes; i++){
        char* _ptr = strchr(labels[i], ':');
        new_labels[i] = (char*)malloc(100 * sizeof(char*));
        strncpy(new_labels[i], labels[i], strlen(labels[i]) - strlen(_ptr));
    }
    return new_labels;
}
 ...
 * https://medium.com/@vivek.yadav/dealing-with-unbalanced-data-generating-additional-data-by-jittering-the-original-image-7497fe2119c3
** 从所有训练图片中, 随机读取n张, 并对这n张图片进行数据增强, 同时矫正增强后的数据标签信息.  最终得到的图片的宽高为w,h(原始训练集中的图片尺寸不定),
 * 也就是网络能够处理的图片尺寸,
** 数据增强包括: 对原始图片进行宽高方向上的插值缩放(两方向上缩放系数不一定相同), 下面称之为缩放抖动;随机抠取或者平移图片(位置抖动);
** 在hsv颜色空间增加噪声(颜色抖动);左右水平翻转, 不含旋转抖动.  
** 输入:  n         一个线程读入的图片张数(详见函数内部注释)
...
**                 具体执行在fill_truth_detection()函数中)
**       classes   类别总数, 本函数并未用到(fill_truth_detection函数其实并没有用这个参数)
**       use_flip  是否使用水平翻转
**       use_mixup 是否使用mixup数据增强
**       jitter    这个参数为缩放抖动系数, 就是图片缩放抖动的剧烈程度, 越大, 允许的抖动范围越大(所谓缩放抖动,
**                 就是在宽高上插值缩放图片, 宽高两方向上缩放的系数不一定相同)
**       hue       颜色(hsv颜色空间)数据增强参数: 色调(取值0度到360度)偏差最大值, 实际色调偏差为-hue~hue之间的随机值
**       saturation 颜色(hsv颜色空间)数据增强参数: 色彩饱和度(取值范围0~1)缩放最大值, 实际为范围内的随机值
**       exposure  颜色(hsv颜色空间)数据增强参数: 明度(色彩明亮程度, 0~1)缩放最大值, 实际为范围内的随机值
...
**       show_imgs
** 返回:  data类型数据, 包含一个线程读入的所有图片数据(含有n张图片)
** 说明:  最后四个参数用于数据增强, 主要对原图进行缩放抖动, 位置抖动(平移)以及颜色抖动(颜色值增加一定噪声), 
**       抖动一定程度上可以理解成对图像增加噪声.通过对原始图像进行抖动, 实现数据增强. 最后三个参数具体用法
**       参考本函数内调用的random_distort_image()函数.
** 说明2: 从此函数可以看出, darknet对训练集中图片的尺寸没有要求, 可以是任意尺寸的图片, 因为经该函数处理(缩放/裁剪)之后,
**       不管是什么尺寸的照片, 都会统一为网络训练使用的尺寸.
*/

data load_data_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int truth_size,
                         int classes, int use_flip, int gaussian_noise, int use_blur, int use_mixup,
                         float jitter, float resize, float hue, float saturation, float exposure,
                         int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound,
                         int contrastive, int contrastive_jit_flip, int show_imgs)
{
    const int random_index = random_gen();
    c = c ? c : 3;
    char **random_paths;
    ...
    float r1 = 0, r2 = 0, r3 = 0, r4 = 0, r_scale;  // 这些参数是干啥用的?
    float resize_r1 = 0, resize_r2 = 0;
    float dhue = 0, dsat = 0, dexp = 0, flip = 0;
    int augmentation_calculated = 0;
    // d.y存储了所有读入照片的标签信息,每条标签包含5条信息: 类别,以及矩形框的x,y,w,h
    // boxes为一张图片最多能够处理(参与训练)的矩形框的数(如果图片中的矩形框数多于这个数,
    // 那么随机挑选boxes个,这个参数仅在parse_region以及parse_detection中出现,好奇怪?
    // 在其他网络解析函数中并没有出现). 同样,d.y是一个matrix,make_matrix会指定y的行数和列数,
    // 同时会为其第一维动态分配内存.
    d.y = make_matrix(n, truth_size * boxes);  // 用于存储标签信息
    int i_mixup = 0;
    for (i_mixup = 0; i_mixup <= mixup; i_mixup++) {
        if (i_mixup) {
            augmentation_calculated = 0;
        }
        for (i = 0; i < n; ++i) {  // 依次读入每一张图片到d.X.vals的适当位置, 同时读入对应的标签信息到d.y.vals的适当位置
            float *truth = (float*)xcalloc(truth_size * boxes, sizeof(float));  // truth 也即为GT
            ...
            if (!augmentation_calculated || !track)
            {
                ...
                if (!contrastive || contrastive_jit_flip || i % 2 == 0)
                {
                    ...
                    flip = use_flip ? random_gen() % 2 : 0;  // 翻转
                }
            ...
            if (resize < 1) {
                ...
                pleft += rand_precalc_random(min_rdw, max_rdw, resize_r1);  // 由于min_rdw, max_rdw均为0, pleft无变化
                pright += rand_precalc_random(min_rdw, max_rdw, resize_r2);
                ptop += rand_precalc_random(min_rdh, max_rdh, resize_r1);
                pbot += rand_precalc_random(min_rdh, max_rdh, resize_r2);
            }
            ...
            float sx = (float)swidth / ow;   // sx也即新的图像宽度与原图宽度的比值
            float sy = (float)sheight / oh;  // sy也即新的图像高度与原图高度的比值
            // crop_image功能是将orig的宽高进行变换到swidth和sheight
            image cropped = crop_image(orig, pleft, ptop, swidth, sheight);

            float dx = ((float)pleft / ow) / sx;  // 等同于: (float)pleft / swidth
            float dy = ((float)ptop / oh) / sy;   // 等同于: (float)ptop / sheight
            // resize到***.cfg配置文件中指定的宽和高.
            image sized = resize_image(cropped, w, h);
            // 翻转
            if (flip) flip_image(sized);
            ...
            //random_distort_image(sized, hue, saturation, exposure);
            // truth包含所有图像的标签信息(包括真实类别与位置, 因为对原始图片进行了数据增强,
            // 其中的平移抖动势必会改动每个物体的矩形框标签信息(主要是矩形框的像素坐标信息),
            // 需要根据具体的数据增强方式进行相应矫正, 后面6个参数就是用于数据增强后的矩形框信息矫正.
            ...
            // 将增强后的图像数据和对应的标签信息, 赋值到d中, d.X中存放的即为图像数据, d.y.vals存放的gt
            d.X.vals[i] = sized.data;
            memcpy(d.y.vals[i], truth, truth_size * boxes * sizeof(float));
...
void *load_thread(void *ptr)
{
    ...
    if(a.exposure == 0) a.exposure = 1;      // 曝光
    if(a.saturation == 0) a.saturation = 1;  // 饱和度
    if(a.aspect == 0) a.aspect = 1;          // 宽高比

    if (a.type == OLD_CLASSIFICATION_DATA){
        *a.d = load_data_old(a.paths, a.n, a.m, a.labels, a.classes, a.w, a.h);
    } else if (a.type == CLASSIFICATION_DATA){  // load 分类数据(基于权重随机采样,同时进行数据增强)
        *a.d = load_data_augment(a.paths, a.n, a.m, a.labels, a.classes, a.hierarchy, a.flip,
                                 a.min, a.max, a.w, a.h, a.angle, a.aspect, a.hue, a.saturation,
                                 a.exposure, a.mixup, a.blur, a.show_imgs, a.label_smooth_eps,
                                 a.dontuse_opencv, a.contrastive, a.sample_weight, a.sample_dicts);
    ...
    } else if (a.type == DETECTION_DATA){  // 导入检测数据
        *a.d = load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.truth_size,
                                   a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter,
                                   a.resize,a.hue, a.saturation, a.exposure, a.mini_batch, a.track,
                                   a.augment_speed, a.letter_box, a.mosaic_bound, a.contrastive,
...
// 互斥锁的类型是pthread_mutex_t, 所以定义一个变量就是创建了一个互斥锁.
// 使用互斥锁, 需要定义并初始化, 初始化一般有两种方式:
// ①:使用函数pthread_mutex_init()对互斥锁进行初始化操作, pthread_mutex_init(&mtx, NULL);
// ②:pthread_mutex_t mtx = PTHREAD_MUTEX_INITIALIZER;
// 参考: https://www.cnblogs.com/zzdbullet/p/9526130.html
...
void *run_thread_loop(void *ptr)
{
    ...
    while (!custom_atomic_load_int(&flag_exit))
    {
        ...
        pthread_mutex_lock(&mtx_load_data);     // 上锁
        load_args *args_local = (load_args *)xcalloc(1, sizeof(load_args));
        *args_local = args_swap[i];
        pthread_mutex_unlock(&mtx_load_data);   // 解锁

        load_thread(args_local);  // 读取图片数据, 核心函数
...
** 流程:  本函数首先会获取要读入图片的张数和要开启线程的个数，而后计算每个线程应该读入的图片张数(尽可能的均匀分配),
...
void *load_threads(void *ptr)
{
    ...
    // 比如train_detector()函数中的args, train_detector()对args非指针变量赋的值都不会受影响, 保持不变.
    // 综其两点, 此处直接free(ptr)是安全的.
    // 说明：free(ptr)函数, 确定会做的事是使得内存块可以重新分配, 且不会影响指针变量ptr本身的值,
    //      也就是ptr还是指向那块地址, 虽然可以使用, 但很危险, 因为这块内存实际是无效的,
    //      系统已经认为这块内存是可分配的, 会毫不考虑的将这块内存分给其他变量, 这样, 其值随时
    //      都可能会被其他变量改变, 这种情况下的ptr指针就是所谓的野指针(所以经常可以看到free之后, 置原指针为NULL).
    //      而至于free(ptr)还不会做其他事情, 比如会不会重新初始化这块内存为0(擦写掉), 以及怎么擦写,
    //      这些操作, 是不确定的, 可能跟具体的编译器有关(个人猜测),经过测试, 对于gcc编译器, free(ptr)之后,
    //      ptr中的非指针变量的地址不变, 但其值全部擦写为0. ptr中的指针变量, 丝毫不受影响, 指针变量本身没有被擦写,
    //      存储的地址还是指向先前分配的内存块, 所以ptr能够正常访问其指针变量所指的值.
    //      测试代码为darknet_test_struct_memory_free.c.
    //      不知道这段测试代码在VS中执行会怎样, 还没经过测试, 也不知道换用其他编译器(darknet的Makefile文件中,
    ...
    if (!threads) {
        ...
        // 变量如果加了voletile修饰，则会从内存中重新装载内容，而不是直接从寄存器中拷贝内容, 因为变量也许会被后台其他程序改变.
        // voletile的典型应用: 详见: https://blog.csdn.net/joy2017/article/details/83583984
        ...
        for (i = 0; i < args.threads; ++i) {
            ...
            *ptr = i;
            // 创建线程, 读取数据, 要注意的是: pthread_create用来创建线程并启动, 也就是创建完后也同时启动了.
            // threads[i] <=> *(threads + i), 这个run_thread_loop相当于旧版本的load_data_in_thread
            if (pthread_create(&threads[i], 0, run_thread_loop, ptr))  // 创建线程并启动
    ...
    for (i = 0; i < args.threads; ++i) {
        ...
        args_swap[i] = args;  // args_swap[i] <=> *(args_swap + i);
        // pthread_mutex_unlock函数用于解锁互斥锁, 一定要记得释放, 不然下一个想要获得这个锁的线程,
        // 就只能去等着了, 如果那个线程很不幸的使用了阻塞等待,那就悲催了(程序卡死)。
        pthread_mutex_unlock(&mtx_load_data);  // 互斥锁解锁
        // run_load_data[i] <=> *(run_load_data + i)
    ...
    for(i = 0; i < args.threads; ++i){
        args.d = buffers + i;  // 每个线程填充的buffers内存空间的指定段
        args.n = (i+1) * total/args.threads - i * total/args.threads;  // 每个线程负责的图片数量
        ...
        // load_data_in_thread里面有pthread_create函数, 创建并启动线程, 最终都是调用load_thread()函数load数据
    ...
    */
    ...
    // 多个线程读入所有数据之后, 分别存储到buffers[0], buffers[1]...中,
    // 接着使用concat_datas()函数将buffers中的数据全部合并成一个大数组得到out
    *out = concat_datas(buffers, args.threads);
    // 也就只有out的shallow敢置为0了, 为什么呢? 因为out是此次迭代读入的最终数据, 该数据参与训练(用完)之后,
    // 当然可以深层释放了, 而此前的都是中间变量,还处于读入数据阶段，万不可设置shallow=0
    out->shallow = 0;
    ...
    // 最终直接释放buffers, 注意buffers是一个存储data的一维数组, 上面循环中的内存释放, 实际是释放每一个data的部分内存
...
/*
** 开辟线程, 读入一次迭代所需的所有图片数据: 读入图片的张数为args.n=net.batch*net.subdivisions*ngpus;
 * darknet总的加载数据的过程:
...
pthread_t load_data(load_args args)
{
    ...
    struct load_args* ptr = (load_args*)xcalloc(1, sizeof(struct load_args));
    *ptr = args;
    // 创建线程并启动, 并绑定load_threads()函数到该线程上, 第二参数是线程的属性,这里设置为0(即NULL),
    // 第四个参数ptr就是load_threads()的输入参数.
...
data load_data_augment(char **paths, int n, int m, char **labels, int k, tree *hierarchy, int use_flip,
                       int min, int max, int w, int h, float angle,float aspect, float hue,
                       float saturation, float exposure, int use_mixup, int use_blur, int show_imgs,
                       float label_smooth_eps, int dontuse_opencv, int contrastive, weights sample_weight,
                       sets* sample_dicts)
{
    char **paths_stored = paths;  // 怎样做到控制batch中的样本比例呢? 模仿pytorch: WeightedRandomSampler(Sampler)
    // 这个地方是随机从总的m条数据中获取n条数据路径, 这个n是每个线程需要load的个数
    // if(m) paths = get_random_paths(paths, n, m);
    // 以下过程是基于权重随机采样
    if(n) paths = get_random_weight_sample_paths(sample_weight, sample_dicts, n);

    data d = {0};
    d.shallow = 0;
    // 导入数据并同时进行数据增强
    d.X = load_image_augment_paths(paths, n, use_flip, min, max, w, h, angle, aspect, hue,
                                   saturation, exposure, dontuse_opencv, contrastive);
    // 导入对应的label
