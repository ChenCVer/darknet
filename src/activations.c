...
float gradient(float x, ACTIVATION a)
{
    switch(a){
        case LINEAR:
            return linear_gradient(x);    // 线性激活函数: y = x.
        case LOGISTIC:
...
/*
** 计算激活函数对加权输入的导数(也即:∂a/∂net=f'(net)), 并乘以delta,得到当前第l层的最终的delta_l(敏感度图)
...
** 说明1: 该函数不但计算了激活函数对于加权输入的导数,还将该导数乘以了之前完成大部分计算的敏感度图delta(所说的大部分
**       也即当前层的delta_l*W_l),因此调用改函数之后,将得到该层最终的敏感度图.
** 说明2: 这里直接利用输出值求激活函数关于输入的导数值是因为神经网络中所使用的绝大部分激活函数,其关于输入的导数值都可以
**       描述为输出值的函数表达式,比如对于Sigmoid激活函数(记作f(x)),其导数值为f(x)'=f(x)*(1-f(x)),因此如果给出
**       y=f(x),那么f(x)'=y*(1-y),只需要输出值y就可以了,不需要输入x的值,(暂时不确定darknet中有没有使用特殊的激活
**       函数,以致于必须要输入值才能够求出导数值,在activiation.c文件中,有几个激活函数暂时没看懂,也没在网上查到).
** 说明3: 关于l.delta的初值,可能你有注意到在看某一类型网络层的时候,比如卷积层中的backward_convolutional_layer()函数,
**        没有发现在此之前对l.delta赋初值的语句,只是用calloc为其动态分配了内存,这样的l.delta其所有元素的值都为0,那么
**        这里使用*=运算符得到的值将恒为0. 是的,如果只看某一层,或者说某一类型的层,的确有这个疑惑,但是整个网络是有很多层的
**        ,且有多种类型,一般来说,不会以卷积层为最后一层,而回以COST或者REGION为最后一层,这些层中,会对l.delta赋初值,又由于
**        l.delta是由后网前逐层传播的,因此,当反向运行到某一层时,l.delta的值将都不会为0.
*/
