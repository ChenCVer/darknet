...
** softmax_layer本身也没有训练参数,所以比较简单,只是darknet中的实现似乎引入
 * 了一些不太常见的东西,导致有些地方理解上比较费劲。
** softmax_layer构建函数
** 输入: batch
**       intputs
**       groups    
** 注意: 此处的softmax_layer层,单张图片的输入元素个数l.inputs等于输出元素
 *      个数l.outputs(总输入元素与总输出元素个数也将相同),
*/
softmax_layer make_softmax_layer(int batch, int inputs, int groups)
{
    ...
    l.cost = (float*)xcalloc(1, sizeof(float));  // 存储损失值
...
** 输入: l   当前softmax层
**       net 整个网络
** 说明: softmax层的前向比较简单,只需要对输入中的每个元素做softmax处理就可以,但是darknet的
 *       实现引入了softmax_tree,这个参数的用法尚需要去推敲。
*/
void forward_softmax_layer(const softmax_layer l, network_state net)
{
    if(l.softmax_tree){
        ...
        for (i = 0; i < l.softmax_tree->groups; ++i) {
            ...
            softmax_cpu(net.input + count, group_size, l.batch, l.inputs, 1, 
                        0, 1, l.temperature, l.output + count);
            count += group_size;
        }
    } else {
...
**      softmax层的反向很简单,由于自身没有训练参数,虽然有激活函数(即softmax函数),但是又因其
 *      所处位置特殊,一般处于网络导数第二层,下一层就是cost层,其自身的敏感度图l.delta已经计算
 *      完成(如果此处不太明白再说什么,可以参看卷积层的注释,或者全连接层的注释,以及最大池化层的注释),
//
**      剩下要做的仅剩下利用自身的l.delta计算其上一层的敏感度图(完成大部分计算,还差乘以上一层激活
 *      函数关于其加权输入的导数值),即将l.delta中的元素乘以对应的当前层与上一次层之间的权重,而
 *      softmax层与上一层输出之间没有权重或者说权重都为1(因为是将输入直接送进softmax函数处理的,
 *      并没有加权什么的),且softmax的输出与上一层的输出存在一一对应的关系,所以求取上一层的敏感度
 *      图也是很简单,很直接,详见下面的注释。
*/
void backward_softmax_layer(const softmax_layer l, network_state net)
{
    // 由当前softmax层的敏感度图l.delta, 值得注意的是, softmax层在前向传播的时候, 已经将l.delta
    // 计算好了, 也即有: l.delta[i]=t-p, 计算上一层的敏感度图net.delta,调用的函数为axpy_cpu(),
    // 为什么调用axpy_cpu()函数,因为softmax层的输出元素与上一层的输出元素存在一一对应的关系(由此
    // 可以看出softmax的stride取值为1是必然的,再次照应blas.c中softmax_cpu()的注释,如果不为1,
    // 肯定不能是一一对应关系),所以由softmax层的敏感度图计算上一层的敏感度图,可以逐个逐个元素计算,
