...
{
    int i, j, k;
    for (i = 0; i < M; ++i) {      // 遍历A的每一行
        for (k = 0; k < K; ++k) {  // 遍历A的每一列
            // register修饰符暗示编译程序相应的变量将被频繁地使用,如果可能的话,应将其保存在CPU的寄存器中,
            // 以加快其存储速度, 提高计算效率.
            // C语言中允许将局部变量的值放在CPU中的寄存器中, 这种变量叫寄存器变量, 我们常用定义变量
            // 存放在内存中!而register是指寄存器变量.寄存器是cpu的存储部件,即是高速缓存, 通常不大,
            // 最多几mb.定义这个变量适用于频繁使用某个变量, 以加快运行速度,因为保存在寄存器中, 省去
            // 了从内存中调用, 要注意定义了这个变量后, 不能取地址!!就是不能使用&符号, 这与一般不同.
            PUT_IN_REGISTER float A_PART = ALPHA * A[i * lda + k];  // 首先将A_PART*A的操作做完
            for (j = 0; j < N; ++j) {    // 使用A_PART的第i行的所有数与B第k列的所有数做乘加操作
                C[i*ldc + j] += A_PART*B[k*ldb + j];
            }
        }
    }
...
void activate_array_cpu_custom(float *x, const int n, const ACTIVATION a)
{
    ...
    {
        for (i = 0; i < n; ++i) {   // n = batch*channel*height*width
            x[i] = (x[i]>0) ? x[i] : .1*x[i];
        }
    }
    else {
        ...
        float *C, int ldc)
{
    ...
    if (is_fma_avx2() && !TA && !TB) {
        ...
        int t;
        /*根据指定的TA和TB来选择不同的矩阵乘法方法,如gemm_nn就代表A与B都不进行转置操作的情况,
         * gemm_tn代表对A进行转置*/
        //当TA=0, TB=0时, 进行的是C = ALPHA * A * B + BETA * C操作
