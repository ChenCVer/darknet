...
layer make_gaussian_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes, int max_boxes)
{
    ...
    l.out_c = l.c;
    l.classes = classes;
    l.cost = (float*)calloc(1, sizeof(float));          // 损失值
    l.biases = (float*)calloc(total*2, sizeof(float));  // 储存anchor值
    if(mask) l.mask = mask;
    else{
...
float delta_gaussian_yolo_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw,
                              int lh, int w, int h, float *delta,float scale, int stride,
                              float iou_normalizer, IOU_LOSS iou_loss, float uc_normalizer,
                              int accumulate, YOLO_POINT yolo_point, float max_delta)
{
    ...
    float tx, ty, tw, th;
    // 对应论文: Gaussian YOLOv3: An Accurate and Fast Object Detector Using Localization
    // Uncertainty for Autonomous Driving
    // tx即为公式(6)中的x_ijk_G, x_G即为truth.x
    tx = (truth.x*lw - i);
    ty = (truth.y*lh - j);
    ...
    // Gaussian, 这里需要着重解释一下, 也是guassian_yolov3的灵魂所在
    // tx = (truth.x*lw - i) = x_ijk_G
    // dx = (x_ijk_G - x_pred), 这里x_pred表示网络预测tx, 相当于dx = (tx_true- tx_pred), 这里的tx_pred就是μ
    // in_exp_x = (x - u)/δ
    float in_exp_x = dx / x[index+1*stride];
    // in_exp_x_2 = [(x - u)/δ]^2
    float in_exp_x_2 = pow(in_exp_x, 2);
    // normal_dist_x即为, 当x = tx_pred时, 高斯分布的概率密度函数值.
    // 如果tx_pred也即μ非常接近tx_true时, 此时概率密度最大.
    // normal_dist_x = [1/[(π*2*δ)]^(1/2)]*e^[(-1./2)*[(x - u)/δ]^2], 即为高斯概率密度函数.
    ...
    // TODO: delta_x这个导数怎么推导出来的??
    float delta_x = temp_x * in_exp_x  * (1. / x[index + 1 * stride]);  // (x-u)/(2*δ^2)
            ...
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    for(t = 0; t < l.max_boxes; ++t){
                        ...
                        int class_id_match = compare_gaussian_yolo_class(l.output, l.classes, class_index,
                    ...
                    if (best_match_iou > l.ignore_thresh) {  // 忽略样本
                        ...
                        if (l.objectness_smooth) {
                            ...
                            delta_gaussian_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w*l.h,
                                                      0, l.label_smooth_eps, l.classes_multipliers);
                        }
                        else l.delta[obj_index] = 0;  // 忽略样本不计算confidence损失
                    }
                    ...
                    if (best_iou > l.truth_thresh) {  // 这个若开启, 会打破cell限制
        ...
        for(t = 0; t < l.max_boxes; ++t){
            ...
            if(mask_n >= 0){
                ...
                float iou = delta_gaussian_yolo_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h,
                                                    ...
                                                    l.iou_loss, l.uc_normalizer, 1, l.yolo_point, l.max_delta);
