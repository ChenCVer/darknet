...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    int groups = option_find_int_quiet(options, "groups", 1);  // 卷积分组
    int size = option_find_int(options, "size",1);  // 卷积核尺寸, 默认卷积核是正方形
    int stride = -1;
    //int stride = option_find_int(options, "stride",1);
    ...
    int dilation = option_find_int_quiet(options, "dilation", 1);  // 空洞卷积
    ...
    char *activation_s = option_find_str(options, "activation", "logistic");
    ACTIVATION activation = get_activation(activation_s);  // get_activation函数返回枚举类型
    ...
    // h, w, c为上一层的输出的高度/宽度/通道数(第一层的则是输入的图片的尺寸与通道数,
    // 也即net.h,net.w,net.c),batch所有层都一样(不变),
    // params.h,params.w,params.c及params.inputs在构建每一层之后都会更新为上一层相应的输出参数
    // (参见parse_network_cfg())
    int batch,h,w,c;
    h = params.h;
    w = params.w;
    ...
    // CBN是BN的一个变种, 主要为了解决BN在小batchsize时效果不好的问题
    // 参考: https://zhuanlan.zhihu.com/p/115949091
    ...
    convolutional_layer layer = make_convolutional_layer(batch,1,h,w,c,n,groups,size,stride_x,stride_y,dilation,
                                                         padding,activation, batch_normalize, binary, xnor,
...
softmax_layer parse_softmax(list *options, size_params params)
{
	...
	// new_onehot_target_labels = onehot_labels * (1 - label_smoothing) + label_smoothing / num_classes
	// 参考: https://www.cnblogs.com/itmorn/p/11254448.html
	// 参考: https://zhuanlan.zhihu.com/p/101553787
	// 参考: https://zhuanlan.zhihu.com/p/73054583
	// 这里的layer.temperature, 这里参考: https://www.jianshu.com/p/cb93d5e39bca
	// output = np.exp(input/T) / np.sum(np.exp(input/T), axis=1)
...
layer parse_gaussian_yolo(list *options, size_params params) // Gaussian_YOLOv3
{
    ...
    l.ignore_thresh = option_find_float(options, "ignore_thresh", .5);  // 忽略样本阈值
    l.truth_thresh = option_find_float(options, "truth_thresh", 1);     // 完全正确样本阈值
    l.iou_thresh = option_find_float_quiet(options, "iou_thresh", 1);   // 正样本阈值
    l.random = option_find_float_quiet(options, "random", 0);
...
maxpool_layer parse_maxpool(list *options, size_params params)
{
    ...
    // 创建maxpool层
    maxpool_layer layer = make_maxpool_layer(batch, h, w, c, size, stride_x,
                                             stride_y, padding, maxpool_depth,
                                             out_channels, antialiasing, avgpool,
                                             params.train);
...
{   // 这个max_batchs也就是网络一共要迭代多少次, 每次迭代都会seen batchsize数目的图片
    ...
    // time_steps是RNN中的概念,即在更新梯度时,不仅仅考虑当前的输入的一个batch,而且还考虑前面time_steps个
    ...
    *net->cur_iteration = 0;   // 当前网络的迭代次数
    ...
    // 网络工作空间的最大容量限制.
    net->workspace_size_limit = (size_t)1024*1024 * option_find_float_quiet(options, "workspace_size_limit_MB", 1024);
    // ADAM优化器相关参数: B1, B2和eps
    net->adam = option_find_int_quiet(options, "adam", 0);  // 如果采用的是adam优化器, 则需要定义B1, B2和eps
    if(net->adam){
    ...
    net->h = option_find_int_quiet(options, "height",0);  // 网络输入图片的高度
    net->w = option_find_int_quiet(options, "width",0);   // 网络输入图片的宽度
    net->c = option_find_int_quiet(options, "channels",0);  // 网络输入图片的通道数
    // 由于数据在内存上是线性存储, net->inputs就表示一张输入图片的内存占有空间量.
    net->inputs = option_find_int_quiet(options, "inputs", net->h * net->w * net->c);
    // max_crop/min_crop: 输入图的最小边需要在[min_crop,max_crop]区间内,如果输入和这个不符合,
    // 则通过缩放使之符合。另外有一对参数max_ratio/min_ratio和一对参数作用是一样的.
    // min_crop默认为net->w; max_crop默认为net->2*w.
    ...
    net->label_smooth_eps = option_find_float_quiet(options, "label_smooth_eps", 0.0f);  // 执行标签平滑操作
    ...
    // 这也是一些数据增强的操作
    net->angle = option_find_float_quiet(options, "angle", 0);     // 图片旋转角度
    net->aspect = option_find_float_quiet(options, "aspect", 1);   // 宽高比
    ...
    net->hue = option_find_float_quiet(options, "hue", 0);  // 色度
    net->power = option_find_float_quiet(options, "power", 4);  // power是啥?

    if(!net->inputs && !(net->h && net->w && net->c)) error("No input parameters supplied");
    // 学习率修改策略
    char *policy_s = option_find_str(options, "policy", "constant");
    net->policy = get_policy(policy_s);
    net->burn_in = option_find_int_quiet(options, "burn_in", 0);  // warm_up
#ifdef GPU
    if (net->gpu_index >= 0) {
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    parse_net_options(options, &net);   // 这里主要是解析***.cfg中的[net]字段部分的配置, 并为net网络赋值相关参数
    ...
    params.h = net.h;  // 网络初始输入尺寸
    params.w = net.w;
    params.c = net.c;
    params.inputs = net.inputs;
    if (batch > 0) net.batch = batch;  // 这个batch是mini_batch
    ...
    n = n->next;  // n此时指向大链中的[net]字段的下一个节点段.也即第一个网络层[convolutional]
    int count = 0;  // 用于顺序记录网络层的编号
    free_section(s);  // 释放[net]字段的配置小链所占有的内存空间
    ...
    while(n){
        ...
        l.clip = option_find_float_quiet(options, "clip", 0);  // 这个参数什么意思
        ...
        net.layers[count] = l;  // 将构建好的网络层添加到net.layers中.
        // 可以看到, 每次构建layer层时, 都会求解l.workspace_size, 然后在此处进行大小比较
        // 因此, 可以看出, workspace_size记录着net中, 工作空间的最大值(也就是数据量最多的那个层)
        if (l.workspace_size > workspace_size) workspace_size = l.workspace_size;
        if (l.inputs > max_inputs) max_inputs = l.inputs;      // max_inputs也是记录这网络中间层输入最大值
        if (l.outputs > max_outputs) max_outputs = l.outputs;  // max_outputs记录着网络中间层输出最大值
        free_section(s);  // 释放空间
        n = n->next;  // 获取大链的下一个节点
        ++count;
        ...
        if(n){  // 递归到最终, 其n为NULL.
            ...
            if (l.antialiasing){
                ...
                params.w = l.out_w;
                params.c = l.out_c;
                params.inputs = l.outputs;   // 下一层的输入即为本层的输出.
            }
        }
        if (l.bflops > 0) bflops += l.bflops;  // bflops为所有网络层的l.blfops之总和.
    ...
    avg_outputs = avg_outputs / avg_counter;  // avg_outputs为平均每个网络层的输出
...
#else
        if (workspace_size) {
            // net.workspace为layer中所有workspace_size的最大值, 然后开辟对应的内存空间
            net.workspace = (float*)xcalloc(1, workspace_size);
        }
#endif
...
{    // cutoff 网络层数, 比如YOLO V2 ---> 网络层编号: 0~31. 这里cutoff则为: 32. 也即需要导入的网络层数.
    ...
    // 在使用多个输出函数连续进行多次输出时, 有可能发现输出错误, 后面加上fflush(stdout), 强制马上输出,避免输出出现延迟.
    fflush(stdout);
