...
int find_int_arg(int argc, char **argv, char *arg, int def)
{
    ...
    for(i = 0; i < argc-1; ++i)
    {
        ...
        if(0==strcmp(argv[i], arg)){
            def = atoi(argv[i+1]);  // 把字符串转换成整型数的一个函数, 该函数要求被转换的字符串是按十进制数理解的
            del_arg(argc, argv, i);
            del_arg(argc, argv, i);
            break;
        }
...
char *basecfg(char *cfgfile)
{
    char *c = cfgfile;
    char *next;
    // strchr函数: 表示在字符串s中查找字符c, 返回字符c第一次在字符串s中出现的位置, 如果未找到字符c, 则返回NULL。
    while((next = strchr(c, '/')))
    {
        c = next+1;
    }
...
{
    float scale = rand_uniform_strong(1, s);  // assert s >= 1
    // 这里实在模拟以0.5的概率输出scale, 以0.5的概率输出1./scale;
...
float rand_precalc_random(float min, float max, float random_part)
{
    ...
    // random_part: 相当于取区间[min, max]之间的多大部分, 其实就是模拟在[min, max]之间随机取一个数
