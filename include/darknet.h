...
struct layer {
    ...
    void(*forward)   (struct layer, struct network_state);  //函数指针
    ...
    int truths;     // < 根据region_layer.c判断,这个变量表示一张图片含有的真实值的个数,对于检测模型来说,一个真实的标签含有5个值,
                    // 包括类型对应的编号以及定位矩形框用到的w,h,x,y四个参数,且在darknet中固定每张图片最大处理30个矩形框,
                    // (可查看max_boxes参数),因此,在region_layer.c的make_region_layer()函数中赋值为30*5.
    ...
    int truth_size;
    int groups;     // 这个参数目前仅发现用在softmax_layer中,含义是将一张图片的数据分成几组,具体的值由网络配置文件指定,
                    // 如未指定默认为1(见parse_softmax()),很多网络都将该值设置为1,相当于没用到该值,我想这可能跟分类与
                    // 分割粒度有关(如果粒度细些,估计会大于1,当然这只是个人猜测)
    int group_id;
    ...
    float temperature;  // 温度参数,softmax层特有参数,在parse_softmax()函数中赋值,由网络配置文件指定,
                        // 如果未指定,则使用默认值1(见parse_softmax())
    float probability;  // dropout概率,即舍弃概率,相应的1-probability为保留概率(具体的使用可参见forward_dropout_layer()),
                        // 在make_dropout_layer()中赋值,
    ...
    */
    ...
    int   * counts;
    float ** sums;
    // 这个参数目前只发现用在dropout层,用于存储一些列的随机数,这些随机数与dropout层的输入元素一一对应,
    // 维度为l.batch*l.inputs(包含整个batch的),在make_dropout_layer()函数中用calloc动态分配内存,
    // 并在前向传播函数forward_dropout_layer()函数中逐元素赋值。里面存储的随机数满足0~1均匀分布,干什么用呢？
    // 用于决定该输入元素的去留,我们知道dropout层就完成一个事：按照一定概率舍弃输入神经元(所谓舍弃就是置该输入的值为0),
    // rand中存储的值就是如果小于l.probability,则舍弃该输入神经元(详见：forward_dropout_layer())。
    // 为什么要保留这些随机数呢？和最大池化层中的l.indexes类似,在反向传播函数backward_dropout_layer()中
    // 用来指示计算上一层的敏感度值,因为dropout舍弃了一些输入,这些输入(dropout层的输入,上一层的输出)对应的敏感度
    // 值可以置为0,而那些没有舍弃的输入,才有必要由当前dropout层反向传播过去。
    float * rand;
    float * cost;   // 目标函数值, 该参数不是所有层都有的, 一般在网络最后一层拥有, 用于计算最后的cost,
                    // 比如识别模型中的cost_layer层, 检测模型中的region_layer层
    int *labels;
    float *cos_sim;
    float *exp_cos_sim;
    ...
    float *biases;  // 当前层所有偏置,对于卷积层,维度l.n,每个卷积核有一个偏置；对于全连接层,维度等于单张输入图片对应的元素个数
                    // 即outputs,一般在各网络构建函数中动态分配内存（比如make_connected_layer()
    ...
    float *col_image;
    float * delta;  // 存储每一层的梯度图:包含所有输出元素的敏感度值(整个batch所有图片)。所谓敏感度,即误差函数关于当前层每个加权输入的导数值,
    ...
    float * output;  // 存储该层所有的输出,维度为l.out_h*l.out_w*l.out_c*l.batch,可知包含整个batch输入图片的输出,一般在构建具体
                     // 网络层时动态分配内存(比如make_maxpool_layer()中)。按行存储:每张图片按行铺排成一大行,图片间再并成一行。
    ...
    tree *softmax_tree;  // softmax层用到的一个参数,不过这个参数似乎并不常见,很多用到softmax层的网络并没用使用这个参数,
                        // 目前仅发现darknet9000.cfg中使用了该参数,如果未用到该参数,其值为NULL,如果用到了则会在parse_softmax()中赋值,
                         // 目前个人的初步猜测是利用该参数来组织标签数据,以方便访问.

    size_t workspace_size;  // net.workspace的元素个数,为所有层中最大的l.out_h*l.out_w*l.size*l.size*l.c,
                            // (在make_convolutional_layer()计算得到workspace_size的大小,在parse_network_cfg()
                            // 中动态分配内存,此值对应未使用gpu时的情况
